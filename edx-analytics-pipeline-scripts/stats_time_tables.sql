...
DROP TABLE IF EXISTS tmp_course_enrolls_daily;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_course_enrolls_daily ON COMMIT PRESERVE ROWS AS
SELECT
...
DROP TABLE IF EXISTS tmp_course_unenrolls_daily;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_course_unenrolls_daily ON COMMIT PRESERVE ROWS AS
SELECT
...
DROP TABLE IF EXISTS tmp_course_verifications_daily;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_course_verifications_daily ON COMMIT PRESERVE ROWS AS
SELECT
...
DROP TABLE IF EXISTS tmp_course_transactions_daily;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_course_transactions_daily ON COMMIT PRESERVE ROWS AS
SELECT
...
DROP TABLE IF EXISTS tmp_course_enrolls_vtr_daily;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_course_enrolls_vtr_daily ON COMMIT PRESERVE ROWS AS
SELECT	
	DATE(c.first_enrollment_time) AS enroll_date_vtr,
	c.course_id,
	s.course_seat_price,
	SUM(CASE 
...
DROP TABLE IF EXISTS tmp_course_certs_daily;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_course_certs_daily ON COMMIT PRESERVE ROWS AS
SELECT
	DATE(modified_date) AS cert_date,
	cert.course_id,
...
DROP TABLE IF EXISTS tmp_course_completions_daily;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_course_completions_daily ON COMMIT PRESERVE ROWS AS
SELECT
	DATE(passed_timestamp) AS completion_date,
	course_id,
...
DROP TABLE IF EXISTS tmp_course_certs_completions_daily;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_course_certs_completions_daily ON COMMIT PRESERVE ROWS AS
SELECT
	COALESCE(
	...
	cnt_certificates
FROM
	tmp_course_completions_daily completions
FULL JOIN
	tmp_course_certs_daily certificates
ON
...
DROP TABLE IF EXISTS tmp_course_vtr_daily;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_course_vtr_daily ON COMMIT PRESERVE ROWS AS
SELECT
	COALESCE(
	...
	course_seat_price
FROM
	tmp_course_enrolls_vtr_daily enrolls
FULL JOIN
	tmp_course_verifications_daily verifications
ON
...
DROP TABLE IF EXISTS pre_course_stats_time;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS pre_course_stats_time ON COMMIT PRESERVE ROWS AS
SELECT
	COALESCE(
		vtr.vtr_date,
		enrolls.enroll_date,
	...
	course_seat_price
FROM
	tmp_course_vtr_daily vtr
FULL JOIN
	tmp_course_enrolls_daily enrolls
ON
	vtr.vtr_date = enrolls.enroll_date
AND
	vtr.vtr_course_id = enrolls.course_id
FULL JOIN
	tmp_course_unenrolls_daily unenrolls
ON
...
DROP TABLE IF EXISTS pre_trans_course_stats_time;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS pre_trans_course_stats_time ON COMMIT PRESERVE ROWS AS
SELECT
	COALESCE(
		pre_cst.date,
...
FROM
	pre_course_stats_time pre_cst
FULL JOIN
	tmp_course_transactions_daily transactions
ON
...
DROP TABLE IF EXISTS pre_calc_course_stats_time;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS pre_calc_course_stats_time ON COMMIT PRESERVE ROWS AS
SELECT
	COALESCE(
		pre_trans_cst.date,
		cert_comp.date
	...
	cnt_certificates
FROM
	pre_trans_course_stats_time pre_trans_cst
FULL JOIN
	tmp_course_certs_completions_daily cert_comp
ON
...
DROP TABLE IF EXISTS course_stats_time;
CREATE TABLE IF NOT EXISTS course_stats_time AS
SELECT
	date,
	course_id,
	cnt_enrolls,
...
FROM
	pre_calc_course_stats_time;
	
--program_stats_time summary table
DROP TABLE IF EXISTS program_stats_time;
CREATE TABLE IF NOT EXISTS program_stats_time AS
SELECT
	date,
	program.course_id,
...
FROM
	production.d_program_course program
LEFT JOIN
	course_stats_time course_stats_time
ON
...
DROP TABLE IF EXISTS tmp_user_reg;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_user_reg ON COMMIT PRESERVE ROWS AS
SELECT
...
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_user_enroll ON COMMIT PRESERVE ROWS AS
SELECT
	DATE(first_enrollment_time) AS enroll_date,
	user_id,
...
DROP TABLE IF EXISTS tmp_user_unenroll;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_user_unenroll ON COMMIT PRESERVE ROWS AS
SELECT
	DATE(last_unenrollment_time) AS unenroll_date,
	user_id,
...
DROP TABLE IF EXISTS tmp_user_verifications;	
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_user_verifications ON COMMIT PRESERVE ROWS AS
SELECT
	DATE(first_verified_enrollment_time) AS verification_date,
	user_id,
...
DROP TABLE IF EXISTS tmp_user_enroll_vtr;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_user_enroll_vtr ON COMMIT PRESERVE ROWS AS
SELECT
...
DROP TABLE IF EXISTS tmp_user_certs;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_user_certs ON COMMIT PRESERVE ROWS AS		
SELECT
	DATE(modified_date) AS cert_date,
	user_id,
...
DROP TABLE IF EXISTS tmp_user_completions;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_user_completions ON COMMIT PRESERVE ROWS AS
SELECT
	DATE(passed_timestamp) AS completion_date,
	user_id,
...
DROP TABLE IF EXISTS tmp_user_transactions;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_user_transactions ON COMMIT PRESERVE ROWS AS	
SELECT
	DATE(transaction_date) AS transaction_date,
	order_username,
	u.user_id,
...
DROP TABLE IF EXISTS tmp_stats_summary_user;
CREATE LOCAL TEMPORARY TABLE IF NOT EXISTS tmp_stats_summary_user ON COMMIT PRESERVE ROWS AS
SELECT
	COALESCE(
		reg_date,
		enroll_date,
...
FULL JOIN
	tmp_user_transactions trans
ON
	reg_date = transaction_date
AND
	reg.user_id = trans.user_id
FULL JOIN
	tmp_user_completions completions
ON
...
DROP TABLE IF EXISTS stats_summary_user;
CREATE TABLE IF NOT EXISTS stats_summary_user AS
SELECT
	absolute_date,
