...
FROM
	production.d_user_course
WHERE
	first_enrollment_time >= '2016-04-26'
GROUP BY
	user_id
...
SELECT
	SUM(CASE WHEN user_account_creation_time >= '2016-04-26' THEN 1 ELSE 0 END) AS acquired_1y
FROM 
	production.d_user;
...
FROM
	(
		...
		FROM
			production.d_user
		WHERE
		    user_account_creation_time >= '2016-04-26'
	) a
JOIN
...
ON
	a.user_id = b.user_id
AND
	b.first_enrollment_time >= '2016-04-26'
...
(
	...
	JOIN
		(
			...
			AND 
				a.first_enrollment_time >= '2016-04-26'
		) c
	ON
		a.user_id = c.user_id
...
DROP TABLE IF EXISTS ahemphill.mimimally_highly_engaged_users_enroll_date_country;
CREATE TABLE IF NOT EXISTS ahemphill.mimimally_highly_engaged_users_enroll_date_country AS
...
(
	...
	ON
		a.user_id = c.user_id
	AND
		a.user_last_location_country_code IN ('CO','MX','ES')
	-- only looking at engaged users, so an inner join is fine
	JOIN
		(
			SELECT
...
--engaged enrollments per course
...
(
	...
	ON
		a.user_id = b.user_id
	AND 
		b.first_verified_enrollment_time >= '2016-04-26'
	AND 
		b.current_enrollment_mode = 'verified'
	JOIN
