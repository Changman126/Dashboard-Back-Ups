...
WITH
  ...
  WHERE
    ...
    AND REGEXP_CONTAINS(hp.v2ProductName, r'{COURSE_TRANSACTION_REGEXP}')
    AND _TABLE_SUFFIX BETWEEN '{EXPERIMENT_START_DATE}' AND '{EXPERIMENT_END_DATE}'
...
SELECT
	...
	exposure.user_id,
	exposure.variation,
	enrollments.course_id AS enrolled_course,
	transactions.transactionId,
	transactions.bookings_usd,
	transactions.course_id AS verified_course
FROM
	exposure exposure
LEFT JOIN
...
AND
	transaction.course_id = enrollments.course_id