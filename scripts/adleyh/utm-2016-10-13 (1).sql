...
DROP TABLE IF EXISTS mulby.utm_touch;
CREATE TABLE mulby.utm_touch AS
SELECT
...
DROP TABLE IF EXISTS mulby.utm_referral;
CREATE TABLE mulby.utm_referral AS
SELECT
...
DROP TABLE IF EXISTS mulby.utm_course_enroll;
CREATE TABLE mulby.utm_course_enroll AS
SELECT DISTINCT
...
DROP TABLE IF EXISTS mulby.utm_course_enroll_attribution;
CREATE TABLE mulby.utm_course_enroll_attribution AS
SELECT DISTINCT
...
FROM mulby.utm_course_enroll v
INNER JOIN mulby.identify i ON (i.user_id = v.user_id AND i.domain = v.domain)
INNER JOIN mulby.utm_touch u ON (u.anonymous_id = i.anonymous_id AND u.domain = v.domain AND u.timestamp < v.timestamp AND DATEDIFF(day, v.timestamp, u.timestamp) > -30)
WHERE
...
DROP TABLE IF EXISTS mulby.utm_registration;
CREATE TABLE mulby.utm_registration AS
SELECT
  CAST(e.timestamp AS TIMESTAMPTZ) AS timestamp,
  e.user_id
FROM experimental_events_run14.event_records e
...
DROP TABLE IF EXISTS mulby.utm_registration_attribution;
CREATE TABLE mulby.utm_registration_attribution AS
SELECT DISTINCT
  v.user_id,
...
FROM mulby.utm_registration v
INNER JOIN mulby.identify i ON (i.user_id = v.user_id)
INNER JOIN mulby.utm_touch u ON (u.anonymous_id = i.anonymous_id AND u.timestamp < v.timestamp AND DATEDIFF(day, v.timestamp, u.timestamp) > -30)
WHERE
  v.timestamp > (TIMESTAMP '2016-09-23 00:00:00' - INTERVAL '30 days')
  AND v.timestamp < '2016-09-23 00:00:00'
;
...
DROP TABLE IF EXISTS mulby.utm_transactions;
CREATE TABLE mulby.utm_transactions AS
SELECT
  t.order_timestamp,
  u.user_id,
  t.order_course_id,
...
-- select CAST(t.order_timestamp AS DATE), sum(t.transaction_amount_per_item), sum(t.order_line_item_quantity) from mulby.utm_transactions t group by 1 order by 1 desc;
-- select sum(t.transaction_amount_per_item) from mulby.utm_transactions t;
 
-- Look for the most recent UTM touch prior to the transaction.
DROP TABLE IF EXISTS mulby.utm_transaction_attribution;
CREATE TABLE mulby.utm_transaction_attribution AS
SELECT DISTINCT
  v.user_id,
...
FROM mulby.utm_transactions v
JOIN mulby.identify i ON (i.user_id = v.user_id)
JOIN mulby.utm_touch u ON (u.anonymous_id = i.anonymous_id AND u.timestamp < v.order_timestamp AND DATEDIFF(day, v.order_timestamp, u.timestamp) > -30 AND u.domain = v.domain)
WHERE
  v.order_timestamp > (TIMESTAMP '2016-09-23 00:00:00' - INTERVAL '30 days')
  AND v.order_timestamp < '2016-09-23 00:00:00'
;
 
 

 
-- SELECT campaign_source, campaign_medium, campaign_name, COUNT(*), SUM(net_bookings) FROM mulby.utm_transaction_attribution GROUP BY 1, 2, 3 ORDER BY 5 DESC LIMIT 10;
-- SELECT campaign_source, campaign_medium, COUNT(*), SUM(net_bookings) FROM mulby.utm_transaction_attribution GROUP BY 1, 2 ORDER BY 4 DESC LIMIT 10;
-- SELECT campaign_source, campaign_medium, partner_short_code, domain AS 'touch_domain', COUNT(*), SUM(net_bookings) FROM mulby.utm_transaction_attribution GROUP BY 1, 2, 3, 4 ORDER BY 6 DESC LIMIT 100;
-- SELECT campaign_source, campaign_medium, domain, COUNT(*), SUM(net_bookings) FROM mulby.utm_transaction_attribution GROUP BY 1, 2, 3 ORDER BY 5 DESC LIMIT 100;
 

-- SELECT domain, campaign_source, campaign_medium, landing_page, course_id, COUNT(*) AS conversions, SUM(net_bookings) AS net_bookings FROM mulby.utm_transaction_attribution GROUP BY 1, 2, 3, 4, 5 ORDER BY 7 DESC LIMIT 100;


DROP TABLE IF EXISTS mulby.campaign_conversions;
CREATE TABLE mulby.campaign_conversions AS
SELECT
  domain AS 'partner',
  ...
  campaign_name,
  net_bookings
FROM mulby.utm_transaction_attribution
UNION ALL
SELECT
  domain AS 'partner',
  ...
  campaign_name,
  CAST(NULL AS FLOAT)
FROM mulby.utm_course_enroll_attribution
UNION ALL
SELECT
  domain AS 'partner',
  ...
  campaign_name,
  CAST(NULL AS FLOAT)
FROM mulby.utm_registration_attribution
;
...
-- FROM mulby.campaign_conversions
-- GROUP BY 1, 2, 3, 4, 5, 6

DROP TABLE IF EXISTS mulby.campaign_performance_by_course;
CREATE TABLE mulby.campaign_performance_by_course AS
SELECT
  partner,
  campaign_source,
  campaign_medium,
...
DROP TABLE IF EXISTS mulby.campaign_performance_by_hour;
CREATE TABLE mulby.campaign_performance_by_hour AS
SELECT
