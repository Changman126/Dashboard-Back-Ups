...
INNER JOIN ahemphill.utm_touch u ON (u.anonymous_id = i.anonymous_id AND u.domain = v.domain AND u.timestamp < v.timestamp AND DATEDIFF(day, v.timestamp, u.timestamp) > -30)
WHERE
  v.timestamp > (TIMESTAMP '2016-11-23 00:00:00' - INTERVAL '120 days')
  AND v.timestamp < '2016-11-23 00:00:00'
;
...
INNER JOIN ahemphill.utm_touch u ON (u.anonymous_id = i.anonymous_id AND u.timestamp < v.timestamp AND DATEDIFF(day, v.timestamp, u.timestamp) > -30)
WHERE
  v.timestamp > (TIMESTAMP '2016-11-23 00:00:00' - INTERVAL '120 days')
  AND v.timestamp < '2016-11-23 00:00:00'
;
...
JOIN ahemphill.utm_touch u ON (u.anonymous_id = i.anonymous_id AND u.timestamp < v.order_timestamp AND DATEDIFF(day, v.order_timestamp, u.timestamp) > -30 AND u.domain = v.domain)
WHERE
  v.order_timestamp > (TIMESTAMP '2016-11-23 00:00:00' - INTERVAL '120 days')
  AND v.order_timestamp < '2016-11-23 00:00:00'
;
 
 

 
-- SELECT campaign_source, campaign_medium, campaign_name, COUNT(*), SUM(net_bookings) FROM ahemphill.utm_transaction_attribution GROUP BY 1, 2, 3 ORDER BY 5 DESC LIMIT 10;
-- SELECT campaign_source, campaign_medium, COUNT(*), SUM(net_bookings) FROM ahemphill.utm_transaction_attribution GROUP BY 1, 2 ORDER BY 4 DESC LIMIT 10;
-- SELECT campaign_source, campaign_medium, partner_short_code, domain AS 'touch_domain', COUNT(*), SUM(net_bookings) FROM ahemphill.utm_transaction_attribution GROUP BY 1, 2, 3, 4 ORDER BY 6 DESC LIMIT 100;
-- SELECT campaign_source, campaign_medium, domain, COUNT(*), SUM(net_bookings) FROM ahemphill.utm_transaction_attribution GROUP BY 1, 2, 3 ORDER BY 5 DESC LIMIT 100;
 

-- SELECT domain, campaign_source, campaign_medium, landing_page, course_id, COUNT(*) AS conversions, SUM(net_bookings) AS net_bookings FROM ahemphill.utm_transaction_attribution GROUP BY 1, 2, 3, 4, 5 ORDER BY 7 DESC LIMIT 100;
